            ...
            <div class="col-12">
                ...
                <div class="card mb-4">
                    <div class="card-header">
                        <h5 class="card-tile mb-0">Add Category</h5>
                    </div>
                    @if($value)
                    <form method="post" action="{{ url('add-category').'/'.$value->id ?? '' }}">
                      @else
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="ecommerce-product-name"
                                    placeholder="Write Category Name" name="category"
                                    aria-label="Category" value="{{$value->name ?? ''}}" required>
                                    @error('category')
                                  <span style="color: red">{{ $message }}</span>
                                  @enderror
                            </div>
                    ...
                    <tbody class="table-border-bottom-0">
                      @foreach($data as $key=> $item)
                        <tr>
                            ...
                            <td>
                              @if($item->status == '1')
                              <a href="{{url('deactive-category').'/'.$item->id ?? ''}}"><span class="badge bg-label-primary me-1">Active</span></a>
                            @else
                            <a href="{{url('active-category').'/'.$item->id ?? ''}}"><span class="badge bg-label-danger me-1">Inactive</span></a>
                            @endif
                            </td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="{{url('add-category').'/'.$item->id ?? ''}}"><i
                                                class="bx bx-edit-alt me-1"></i> Edit</a>
                                        <a class="dropdown-item delete-confirm" href="{{url('delete-category').'/'.$item->id ?? ''}}"><i class="bx bx-trash me-1"></i>
                                            Delete</a>
                                    </div>
                                </div>
                            </td>
